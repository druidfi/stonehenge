...
# Stonehenge
#
FROM traefik:v3.7.10 AS stonehenge
...
RUN apk --update --no-cache add bind-tools curl nginx openssh socat sudo tzdata && \
    curl -fsSL -o /usr/local/bin/mkcert "https://dl.filippo.io/mkcert/latest?for=linux/${TARGETARCH}" && \
